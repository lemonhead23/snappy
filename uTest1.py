...
class SNET_makeoffer(SNET_BaseTest, SNET_apicalls):
...
class SNET_placeask(SNET_BaseTest, SNET_apicalls):
...
class SNET_placeask_full(SNET_BaseTest, SNET_apicalls):
...
class SNET_placebid(SNET_BaseTest, SNET_apicalls):
...
class SNET_placebid_full(SNET_BaseTest, SNET_apicalls):
...
class SNET_allorderbooks(SNET_BaseTest, SNET_apicalls):
    """
    ...
    def myXtraStuff(self):
        doSomeStuff=1+2


class SNET_openorders(SNET_BaseTest, SNET_apicalls):
    """
...
class SNET_orderbook(SNET_BaseTest, SNET_apicalls):
...
class SNET_bid(SNET_BaseTest, SNET_apicalls):
...
class SNET_ask(SNET_BaseTest, SNET_apicalls):
...
class SNET_allsignals(SNET_BaseTest, SNET_apicalls):
...
class SNET_lottostats(SNET_BaseTest, SNET_apicalls):
...
class SNET_tradehistory(SNET_BaseTest, SNET_apicalls):
...
class SNET_getsignal(SNET_BaseTest, SNET_apicalls):
...
class SNET_cancelquote(SNET_BaseTest, SNET_apicalls):
...
class SNET_processjumptrade(SNET_BaseTest, SNET_apicalls):
...
class SNET_jumptrades(SNET_BaseTest, SNET_apicalls):
...
class SNET_respondtx(SNET_BaseTest, SNET_apicalls):
...
class SNET_processutx(SNET_BaseTest, SNET_apicalls):
...
def suite_idexSuite():
    ...
    suite.addTest(SNET_processjumptrade())
...
def main():
    ...
    try:
        if args[0] == 'all': # should run all tests in the main class
            unittest.main()
    except:
        print(main.__doc__)
...
char *SuperNET_json_commands(struct NXThandler_info *mp,char *previpaddr,cJSON *origargjson,char *sender,int32_t valid,char *origargstr)
{
  ...
  //static char *processjumptrade[] = { (char *)processjumptrade_func, "processjumptrade", "V", "assetA", "amountA", "other", "assetB", "amountB", "feeA", "feeAtxid", "triggerhash", "jumper", "jumpasset", "jumpamount", "balancing", "balancetxid", "gui", "quoteid", 0 };
    ...
    getpeers, maketelepods, tradebot, respondtx, checkmessages, openorders, allorderbooks, placebid, bid, placeask, ask, sendmsg, sendbinary, orderbook,
